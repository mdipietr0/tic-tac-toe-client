...
var flash = function flash(success, msg) {
...
var onSignUp = function onSignUp(e) {
  ...
  api.signUp(data).then(ui.onSignUpSuccess).then(function () {
...
var onSignOut = function onSignOut(e) {
  ...
  api.signOut().then(ui.onSignOutSuccess).catch(ui.onSignOutSuccess);
};
...
var onGetAllGames = function onGetAllGames(e) {
  ...
  api.index().then(ui.onGetAllGamesSuccess).catch(ui.onGetAllGamesFailure);
};
...
var onCreateGame = function onCreateGame(e) {
  ...
  }).then(ui.onCreateGameSuccess).catch(ui.onCreateGameFailure);
};
...
var onShowGame = function onShowGame(e) {
  ...
  api.show(id).then(function (response) {
    store.game = new Game(response.game);
    store.game.getCurrentPlayer();
    ui.initYourTurn(store.game.player.toUpperCase());
    ui.updateBoard(store.game.cells);
  }).then(ui.onShowGameSuccess).catch(ui.onShowGameFailure);
};

var onUpdateGame = function onUpdateGame(e) {
  console.log('update nodename' + e.target.nodeName);
  e.preventDefault();
  console.log('game events onShowGame');
  var data = getFormFields(e.target);
  console.log(data);
  api.update(data).then(function (response) {
    store.game = new Game(response.game);
    ui.updateBoard(store.game.cells);
    ui.changeTurn();
...
var getSquareNum = function getSquareNum(target) {
  return target.id[target.id.length - 1];
};

var updateBoard = function updateBoard(response) {
  store.game.cells = response.game.cells;
  ui.updateBoard(store.game.cells);
};

var onBoxClick = function onBoxClick(e) {
  var squareNum = getSquareNum(e.target);
  var data = store.game.makeMove(squareNum);
  if (data) {
    api.update(data).then(updateBoard).then(ui.changeTurn).then(ui.onUpdateGameSuccess).then(checkForWinner).then(store.game.changePlayer.bind(store.game)).catch(ui.onUpdateGameFailure);
  }
};

var onMainMenu = function onMainMenu() {
  ui.onMainMenu();
};

var calculateStats = function calculateStats(response) {
  console.log('test');
...
var onTeamSelect = function onTeamSelect(e) {
  var team = $(this).attr('data-team');
  console.log('test ' + team + ' select');
  var player = $('#teamModal').attr('data-player');
  console.log(player);
...
var setTeamToken = function setTeamToken(team, player) {
...
var loadTeams = function loadTeams() {
  var html = '';
  for (var imgName in config.imgUrls) {
    if (imgName === 'player1' || imgName === 'player2' || imgName === config.teamNames.player1 || imgName === config.teamNames.player2) {
      ...
      var image = config.imgUrls[imgName];
      console.log(imgName);
...
var changeTurn = function changeTurn() {
  ...
  var player = yourTurn.attr('data-player');
...
var onGetAllGamesSuccess = function onGetAllGamesSuccess(response) {
  ...
  var html = '<h4 class="col-md-offset-5 col-md-2 text-center align-center">Select a game</h4>';
  console.log(response.games);
  response.games.filter(function (game) {
    return game.over === false;
  }).reduce(function (acc, game) {
...
var initYourTurn = function initYourTurn(player) {
...
var onCreateGameSuccess = function onCreateGameSuccess() {
...
var onShowGameFailure = function onShowGameFailure() {
...
var onWin = function onWin(winner) {
  setTimeout(function () {
    winner = winner.toUpperCase();
    console.log('game over, ' + winner + ' wins');
    var winCount = parseInt($('#counter' + winner).text());
    winCount += 1;
    console.log(winCount);
    $('#counter' + winner).text(winCount);
    var teamText = winner === 'X' ? config.teamNames[config.teamNames.player1] : config.teamNames[config.teamNames.player2];
    var text = teamText + ' Win!!!';
    onGameOver(text);
  }, 300);
};
...
var onGameOver = function onGameOver(text) {
...
var setStats = function setStats() {
  $('#total-games').text(arguments.length <= 0 ? undefined : arguments[0]);
  $('#games-finished').text(arguments.length <= 1 ? undefined : arguments[1]);
  $('#games-drew').text(arguments.length <= 2 ? undefined : arguments[2]);
  $('#games-won').text(arguments.length <= 3 ? undefined : arguments[3]);
  $('#games-lost').text((arguments.length <= 1 ? undefined : arguments[1]) - (arguments.length <= 2 ? undefined : arguments[2]) - (arguments.length <= 3 ? undefined : arguments[3]));
};
...
var Game = function Game() {
  var game = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : { cells: ['', '', '', '', '', '', '', '', ''], over: false };
...
var didWin = function didWin() {
  console.log(this);
  if (this.cells[0] !== '' && this.cells[0] === this.cells[1] && this.cells[1] === this.cells[2] || this.cells[3] !== '' && this.cells[3] === this.cells[4] && this.cells[4] === this.cells[5] || this.cells[6] !== '' && this.cells[6] === this.cells[7] && this.cells[7] === this.cells[8] || this.cells[0] !== '' && this.cells[0] === this.cells[3] && this.cells[3] === this.cells[6] || this.cells[1] !== '' && this.cells[1] === this.cells[4] && this.cells[4] === this.cells[7] || this.cells[2] !== '' && this.cells[2] === this.cells[5] && this.cells[5] === this.cells[8] || this.cells[0] !== '' && this.cells[0] === this.cells[4] && this.cells[4] === this.cells[8] || this.cells[2] !== '' && this.cells[2] === this.cells[4] && this.cells[4] === this.cells[6]) {
    this.gameOver();
    return true;
  }
  return false;
 ...
 *   square number {0,8}
 */
var isSquareAvailable = function isSquareAvailable(index) {
  console.log(this);
  if (this.cells[index] === '') {
    return true;
  }
 ...
 */
var drawBoard = function drawBoard(game) {
  game.cells.forEach(function (s) {
